...
function drawGrid(panel) {
  ...
  // MTF Dashboard rows: horizontal price gridlines with labels, plus
  // vertical gridlines for visual structure — but no date/time TEXT under
  // those vertical lines. This distinction matters and was worth getting
  // right rather than assumed: the reference image shows faint vertical
  // dividers in every single row, just without a date label beneath them —
  // "no date labels" means exactly that, not "no vertical gridlines at
  // all." An earlier version of this skipped vertical gridlines entirely
  // for dashboard panels, which doesn't match the reference. Keyed to
  // WHICH panel this is (not pixel height) for the same reason as before —
  // a height-based check would silently stop applying if row height changes.
  ...
  const vDivisions = isDashboardPanel ? 3 : 4;
  for (let i = 0; i <= vDivisions; i++) {
    const t = panel.viewT0 + (panel.viewT1 - panel.viewT0) * i / vDivisions;
    const x = panel.timeToX(t);
    ctx.beginPath(); ctx.moveTo(x, panel.padT); ctx.lineTo(x, panel.H - panel.padB); ctx.stroke();
    if (isDashboardPanel) continue; // vertical line only — no date/time text for dashboard rows
